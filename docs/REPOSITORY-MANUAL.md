...
| `requirements-connector.in` | Optional direct dependency for the supported Codex/Claude native-peer connector. |
| `requirements-governance.txt`, `requirements-dev.txt` | Hash-locked transitive dependency sets generated by `pip-compile`. |
| `requirements-connector.txt` | Separately hash-locked optional Agent SDK connector runtime generated by `uv pip compile`. |
...
| `docs/protocol/app-quickstart.md` | Desktop-first setup, native capability comparison, supported transient cross-app relay, and durable communication. |
| `docs/protocol/claude/task-connector.md` | Exact supported Agent SDK bridge architecture, tools, install path, relay semantics, and private-Desktop exclusions. |
...
| `coordination/bin/claude-task-connector` | Selects the checkout/primary-worktree Python runtime and starts the local MCP adapter; MCP startup itself does not launch Claude. |
...
Transient Codex/Claude communication defaults to the named
`pipeline-codex-bridge` Agent SDK peer described in
`docs/protocol/claude/task-connector.md`. Formal or durable state still uses the
fixed mailbox path.
