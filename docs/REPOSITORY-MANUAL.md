...
| `.cursor/` | 9 | Cursor app rules, agents, hook, and review skill. |
| `.codex/` | 9 | Codex agent definitions and configuration. |
| `.agy/` | 5 | AGY agent definitions. |
...
`scripts/system_health_check.py`; it migrates AGY's obsolete `.agy/agents/`
TOML catalog into the host-discovered `.agents/agents/` Markdown surface and
adds focused regression modules. Counts in
...
| `.env.example` | Optional external-runner variables only. It excludes role/runtime identity, secrets, and `GIT_INDEX_FILE`; provider-specific runtime mechanisms such as launchers or Cursor `sessionStart` binding own those facts. |
...
|---|---|
| `.agents/skills/` | Canonical reusable skills: four-seat routing, role deltas, claim/control probes, regression pins, wave gate, variable isolation, AGY harness, consultation, and domain-specific Kurogane guidance. Skill presence grants no role. |
...
| `.agents/agents/`, `.agents/workflows/` | AGY-native read-only advisor definitions and the saved read-only `/pipeline-start` workflow; formal roles still require explicit assignment. |
| `.cursor/rules/`, `.cursor/agents/`, `.cursor/skills/` | Cursor app rules, advisors, and the `review-next` UI workflow. |
| `.cursor/hooks.json`, `.cursor/hooks/seat-policy` | The sole repository lifecycle-hook plane, used to gate real Cursor app write/approval surfaces. |
...
| `coordination/bin/` | Ten fixed shell front doors for launch, mailbox, locks, Cursor effects, and claim probing. |
...
| `docs/protocol/{codex,claude,agy,cursor}/` | Host-specific continuation/adoption mechanics. Cursor also has per-role pages. |
| `docs/protocol/threeway/` | Optional signed-plane onboarding, adoption, architecture, mechanism ledger, and two historical reviews. |
...
| `scripts/agy_protocol_model.py` | AGY-specific runtime identity adapter. |
| `scripts/agy_seat_launcher.py` | Validates per-seat AGY model/effort, checks the live model listing, restricts forwarded flags, sanitizes environment, and launches in the selected repository. |
| `scripts/agy_emit.py` | AGY convenience wrapper over `send-event`; the candidate requires an explicit assigned sender and stages without committing. |
| `scripts/agy_observer.py` | Read-only signed-bus summary for an AGY observer. |
| `scripts/cursor_protocol_model.py` | Maps Cursor naming onto the canonical runtime contract. |
| `scripts/cursor_app_binding.py` | Resolves a Cursor Desktop app seat from linked worktrees and session metadata; `sessionStart` atomically updates its user-local registry. |
| `scripts/cursor_seat_launcher.py` | Read-only Cursor seat registry/status diagnostics. |
| `scripts/cursor_hook_policy.py` | Cursor app shell/file/MCP policy; it denies unenforceable non-Director file-tool asks, asks on every well-formed MCP invocation without treating the card as effect authority, and blocks direct protected-ref writes. |
| `scripts/cursor_mailbox.py` | Cursor app mailbox front door delegating publication/consume effects to fixed writers. |
| `scripts/cursor_review_snapshot.py` | Materializes one committed review head into bounded scratch without mutating Git. |
| `scripts/cursor_land_gate.py` | Runs the Cursor app-seat unit gate before a readiness claim. |
| `scripts/harness_preflight.py` | Checks whether a proposed review harness can execute its required path before dispatch; `--live` may launch a provider and incur spend, so it remains separately authorized. |
...
| `coordination/bin/agy-seat` | Capability-checks Python/tomllib and invokes `agy_seat_launcher.py`; provider launch is still separately authorized. |
| `coordination/bin/cursor-seat` | Read-only Cursor seat diagnostics. |
...
| `coordination/bin/cursor-publish` | Cursor app front door for a separately approved event publication. |
| `coordination/bin/cursor-consume` | Cursor app front door for a separately approved cursor advance. |
...
| AGY | `/pipeline-start`, host-discovered `.agents/agents/`, or `coordination/bin/agy-seat` | Native peer/helper relay is transient; parent helpers do not inherit formal authority. |
| Cursor | Agents Window, linked app worktrees, `review-next`, and `seat-policy` hook | File-tool asks are not enforced by Cursor, so only bound Directors edit that way; shell/MCP approvals use enforceable hooks. |
...
| Provider model list unavailable or configured model absent | Refuse launch | `agy_seat_launcher.py` |
| Forwarded CLI flag overrides fixed launch identity/workspace/execution shape | Refuse launch | Codex/AGY launchers; ambient trusted config still owns effective posture |
| Protected Cursor ref written through raw Git plumbing | Deny command | `cursor_hook_policy.py` |
...
|---|---|---|
| AGY changed `models` output to `id<TAB>display`; the launcher stored the whole line. | Every configured ID could be rejected despite being listed. | Parse both ID-only and current tab-separated output; reject malformed rows; add regression tests. |
...
| Cursor policy protected paths but allowed raw protected-ref writes, execution-bearing reader options/environment, spoofed reader executables, and substring-matched mailbox reads. | Raw plumbing, disguised execution, or a crafted publication could bypass fixed writers and child/top-level approval boundaries. | Deny protected ref rewrites, unsafe Git dispatch/environment/options, untrusted executable provenance, unresolved shell expansion, and non-exact read-only mailbox forms. |
...
| `agy_emit.py` defaulted to the invalid pseudo-sender `agy` and committed automatically. | It could not publish lawfully and collapsed staging/commit authority. | Require an explicit assigned sender; fixed writer stages; commit stays separate. |
...
| Active doctrine and provider copies repeated retired four-seat ceremony, hook/index rules, blanket model-independence, and Cursor lock-approval claims that contradicted a hard deny. | Agents could follow stale prose over current risk-based executable behavior. | Reduce provider copies to adapters, remove retired hooks/duplicates, and align active doctrine with explicit roles, native worktrees, risk-gated independence, and actual effect/deny paths. |
...
2. **Centralize effect/destination classification without centralizing all
   ...
   the Cursor hook, launchers, admission gate, writer, and signed gate. A small
...
| Dry-run provider launch spec | `coordination/bin/codex-seat --dry-run <seat>` or `coordination/bin/agy-seat --dry-run <seat>` | AGY dry-run still queries the provider; launch/provider access remains separately authorized. |
