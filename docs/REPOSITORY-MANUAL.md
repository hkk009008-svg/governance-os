...
| `pipeline/peer.py` | Runs one peer CLI once under a timeout and commits a receipt of what ran. Refuses a `--task` that is not one safe path component, never reuses a sequence number, and creates the receipt exclusively. |
...
| `pipeline/harness_preflight.py` | Reports whether both peer CLIs are on PATH and whether the project config carries ambient authority or any declared MCP server; `--live` may launch a provider and incur spend, so it remains separately authorized. |
...
one-shot peer invocation described in `docs/protocol/peer.md`: the other CLI
is run once as a child process and a receipt records what ran. Formal or
durable state still uses the
fixed mailbox path.
...
No CI job writes to the repository; the workflow is verification only, with a
...
8. **Retire dormant commands only after call-path proof.** The signed-bus emitters
