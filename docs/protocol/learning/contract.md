...
> Loaded on trigger when learning-plane work (index, candidates, extraction,
> metrics, skill distribution) is named. Normative copy of the plan's §2/§3/§4/§7
...
- **I1 — learning outputs are advisory inputs.** Recalled memory, index
  results, skills, and candidates never bind a decision. Mechanized core: an
  import test asserts `scripts/mailbox_writer.py` and
  `scripts/compact_pair_loop.py` import no `learning_*` module. Everything
  else in I1 is doctrine: any future import of a learning module into those
  two files is a contract change and reviews as one. Live task state is read
  from durable shared state — Git, mailbox, status — never from the index
  (`scripts/codex_protocol_model.py:17`, CENTRAL_INVARIANT).
- **I2 — no learning artifact grants authority.** No memory, skill, or
  candidate grants a seat, approval, push, merge, spend, or launch.
  Mechanized by absence: the fixed writer and compact-pair validators consume
  no candidate authority field (`scripts/mailbox_writer.py:165-209`). No
  parser-shape assertion exists for this — a text classifier over agent prose
  is what guard admission forbids (`docs/protocol/agents/core.md:163-191`).
- **I3 — canonical writes travel the governed path.** Autonomous learning
  produces immutable candidates only. Promotion IS the compact pair: an
  ordinary git change whose verify-request lists the candidate ref, reviewed
  at the change's risk class. There is no second approval object.
- **I4 — fail-closed disposition binds at Stage 2b, not before.** Until the
  writer-side promotion branch lands in
  `scripts/mailbox_writer.py:validate_event_candidate`, a stale or
  self-approved `decision` publishes durably and is caught only if a reader
  runs the parser. Every pre-2b surface must say "advisory" where it
  describes refusals. The target-base-hash CAS compares bytes at the
  disposition event's own commit, never a live worktree — promotion changes
  worktree bytes by design.
- **I5 — the governance floor lives in operator judgment.** A
  ...
  verify-request (`AGENTS.md` authority floor) — not by candidate machinery,
  because `_finding_refs` is a shape-only regex
  (`scripts/compact_pair_loop.py:250-266`).
- **I6 — trusted mutation needs fail-closed backup.** Nothing in the learning
  plane performs archival or destructive maintenance. Where such a change is
  eventually filed (standalone, per plan §5 Stage 6), backup-failure-blocks
  and recoverability are new behavior to build; `scripts/archive_handoffs.py`
  is fail-open and is not a pattern to inherit.
- **I7 — every proposed guard passes guard admission.** Name the effect it
  blocks, survive the bypass question, or do not build it
...
- **Committed shared knowledge** (repository scope): `docs/**`,
  `coordination/mailbox/sent/**` + `kinds.txt`, `logs/**` including
  `logs/claims/ledger.jsonl`, both skill trees. The `.gitignore` partition is
...
- **Private profile memory** (user scope): `~/.claude` memory dirs, global
  ...
  episodic index must not ingest it.
- **Scoped episodic index** (workspace scope): derived, rebuildable,
  ...
  ref-bus, presence heartbeats, locks). Never committed.
...
verdicts. Candidate ID is a content hash, so a duplicate is byte-idempotent
and Supersedes handles replacement; the refusal may simply drop.
...
Sources: the codex-side Hermes 0.19.0 study (verified defects, snapshot paths
cited in the plan) and this repo's own measured vacuity classes.
