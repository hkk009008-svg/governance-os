...
## Git hygiene (Cursor worktree)
Use the worktree's native Git index; never set GIT_INDEX_FILE. Read-only git
only (show / log / diff / grep). Run pytest normally. You do not edit or
commit — you report.
...
- spent in a **phase the precheck doesn't gate** (e.g. pod start/run/stop
  ...
  any phase that spends AFTER the precheck ran).

For the diff under review, prove every new/changed spend reaches the *same*
accumulator the gate reads — grep the production WRITE site, not the type
declaration.
...
## Method
1. `git show <SHA>` — read the real diff.
...
4. Sibling sweep: other spend paths on the same accumulator that need the same
   fix.
