...
- **Seat Launchers (Default Behavior):** Seats operate as separate chat instances launched via `coordination/bin/agy-seat --mode single-model-autonomous <seat>`. Communicating recipient seats process committed mailbox events emitted via `scripts/agy_emit.py`.
