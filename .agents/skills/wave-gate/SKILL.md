...
row is accepted into a wave, there is nothing here to gate, and a GATE-MET
claim would have no subject. Use this skill when a row exists, not to
manufacture one.
...
reconciliation. Owner: the role that owns the wave.
Re-evaluate: if a MET wave still has open rows on `origin/main`.
