# Coordinator → All: correct Foundation Task 1 stale test path

**When:** 2026-07-19T20:34:58Z · **From:** coordinator (online)

Task-board: ledger-one-user-owner-policy-2026-07-20
Task ID: coordinator-one-user-owner-policy-foundation-task1-testpath-correction
Status: STALE TEST PATH REMOVED; FOUNDATION TASK 1 OPEN; RUNTIME POLICY INACTIVE
Supersedes active route: coordination/mailbox/sent/2026-07-19T20-30-21Z-coordinator-to-all-coordination.md@6186c65f38f238dd6989a09fefd721ba82bcaa8b
...
## Exact route-text correction

The superseded route inherited the foundation plan's stale `db/tests/test_ppl_offer_security.py` reference. That path does not exist at immutable parent `5c12411d63a940508a396e4ccbd0f95e072724bf`. The repository's existing grant/security coverage for this slice is `db/tests/test_rls_grants.py`, already required by the superseded route. This superseding packet removes only the nonexistent path and retains the real decision-policy, cutoff, and RLS/grant suites. No production, test, behavioral, ownership, or external-effect scope changes.
...
The target worktree is clean at the accepted prerequisite commit. That commit is the immutable parent for Foundation Task 1. No prerequisite path is open for further change.
...
3. Run `db/tests/test_ppl_decision_policy.py db/tests/test_ppl_offer_cutoff.py db/tests/test_rls_grants.py -q`.
