# Coordinator → All: route audit finding 5 outcome integrity correction

**When:** 2026-07-20T18:45:54Z · **From:** coordinator (online)

Task-board: pipeline-audit5-abandoned-takeover-outcome-integrity-2026-07-21
Task ID: pipeline-audit5-abandoned-takeover-outcome-integrity-2026-07-21
Status: ACTIVE — APPROVED TWO-FILE OUTCOME-INTEGRITY CORRECTION ROUTED
Authorization source: user-task:approved-governed-director-route-for-audit5-2026-07-21
Pipeline control HEAD before publication: edc7a6a8f7974499aea30843d48c039596a16b0d
...
Binding finding ref: docs/superpowers/specs/2026-07-21-abandoned-takeover-outcome-integrity-design.md@a1655ec77163e486af2f6a546ce266d6e20cc3e5
...
The reproduced authority-integrity defect is accepted for bounded remediation. The canonical model already rejects an abandoned takeover that changes the outcome, but the dispatch-claim adapter omits the candidate-versus-parent outcome delta before invoking that guard. Director restores adapter parity and adds the paired unchanged-versus-changed regression specified by the approved plan.

This route covers audit finding 5 only. It does not change takeover policy, schemas, mailbox formats, ownership semantics, or any other audit finding.

## Director Autonomous Contract Revision 1

Before editing either implementation path, Director publishes exactly one director-to-all coordination event through the fixed writer and commits only that generated event. It uses these exact autonomous fields:
...
- Parent contract: this committed Coordinator route's exact path at its full commit SHA
- Contract revision: 1
- Previous owners: director
- Owners: director
- Proposal ref: self-candidate
- Acceptance refs: self-candidate
- Finding refs: docs/superpowers/specs/2026-07-21-abandoned-takeover-outcome-integrity-design.md@a1655ec77163e486af2f6a546ce266d6e20cc3e5

Director proves the committed autonomous route effective before production editing. If the parent, revision, finding ref, owner identity, model identity, allowed paths, or live Pipeline state differs, Director makes no production edit and reports the exact blocker to Coordinator.
...
1. Director reads the approved design and plan, refreshes Pipeline HEAD and status, and preserves unrelated peer work.
2. Director follows the approved plan test-first. The new changed-outcome parameter must produce the specified RED while the unchanged case remains green.
...
6. Director publishes and commits one canonical verify-request assigned only to Operator2. The request binds the actual implementation base/head, exact two-file manifest, author and reviewer seat/model identities, approved design and plan, binding finding ref, RED evidence, GREEN evidence, and smoke evidence.
...
One Director autonomous-contract event and its exact local Pipeline commit are authorized before implementation.
