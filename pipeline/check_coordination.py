...
_ARCHIVE_DIRECTORIES = frozenset(
    {
        ...
        "pipeline",
        "pipeline/baselines",
        # Pre-rename twins: this projection reads COMMITTED history, and
        # history predating the scripts/ -> pipeline/ move still carries the
        # manifests at their old path. See _legacy_twin below.
        "scripts",
        "scripts/baselines",
    }
)
...
_ARCHIVE_REPORT_BASELINE = "pipeline/baselines/lane_v_reports_pre_v3.json"
_ARCHIVE_HISTORY_EXCEPTIONS = (
    "pipeline/baselines/immutable_review_history_exceptions.json"
)
    ...
    try:
        with tarfile.open(fileobj=io.BytesIO(raw_archive), mode="r:") as archive:
            for member in archive:
                ...
                canonical = _normalize_archive_name(name)
                if (
                    canonical not in {
                        _ARCHIVE_KINDS_PATH,
                    ...
                    and not canonical.startswith(_ARCHIVE_SENT_PREFIX)
                ):
...
def _parse_history_exceptions(
    ...
    for entry in value["entries"]:
        ...
        if is_report and (
            ...
            or _normalize_archive_name(exception.digest_authority)
            != _ARCHIVE_REPORT_BASELINE
        ):
...
def _committed_mailbox_projection(
    ...
    for token in history.stdout.split(b"\0"):
        ...
        if descriptor.startswith(":"):
            ...
            continue
        value = _normalize_archive_name(value)
        if (
            (
    ...
    # A manifest resolving to neither prefix is a DELETION and stays fatal;
    # resolving to both is an ambiguous half-rename and is refused too. The
    # twin makes an old commit readable, never a missing manifest tolerable.
    for required in (_ARCHIVE_REPORT_BASELINE, _ARCHIVE_HISTORY_EXCEPTIONS):
        matched = [
            candidate
            for candidate in manifest_paths
            if _normalize_archive_name(candidate) == required
        ]
        if len(matched) != 1:
            return None, (
                f"committed manifest {required} resolves to {len(matched)} paths "
                f"at {pinned_head}; expected exactly one"
            )
