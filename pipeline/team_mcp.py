...
from team_store import (
    MAX_BODY_BYTES, MAX_MESSAGE_ID, MAX_READ_LIMIT, MAX_STATUS_SENT, MAX_WAIT_SECONDS,
    RECIPIENTS, STATUS_SENT_DEFAULT, TeamError,
)
...
TOOLS = [
    {
        ...
        "description": (
            ...
            "the caller's resume_cursor (the after_id that makes team_wait return "
            "exactly the unread messages), and recent sent-message previews with "
            "cursor-acknowledgement/reply state (10 by default, sent_limit up to 50). "
            "Pass message_id to read one own sent message in full, including older messages. "
            "This does not acknowledge inbound messages. Activity is not liveness or "
            "authority; member labels are configured self-labels, not attestation."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
...
class McpServer:
    ...
    def call_tool(self, name: object, arguments: object) -> dict:
        try:
            if name == "team_status":
                values = self._arguments(arguments, {"message_id", "sent_limit"})
                for key in ("message_id", "sent_limit"):
                    if key in values and values[key] is None:
                        raise TeamError(f"{key} must be an integer")
