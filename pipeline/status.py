#!/usr/bin/env python3
"""pipeline/status.py — live "where are we" snapshot of the repo/program state.

Usage
-----
  python pipeline/status.py snapshot             # compact Codex orientation
  python pipeline/status.py snapshot <seat>      # one assigned role
  python pipeline/status.py                      # compatibility dashboard
...
Repo root is resolved as the parent of this file's parent (pipeline/ → repo/).
"""
...
# pipeline/ is on sys.path when status.py runs (own dir as a script, or inserted
...
def render(data: dict) -> str:
    ...
    a("  source:  pipeline/check_doc_claims.py (ARCHITECTURE.md)")
    a("")
    ...
    # --- Smoke pointer ---
    a("## Smoke test")
    a("  smoke: run `python pipeline/governance_verify_all.py`")
...
def collect_doc_integrity(repo_root: Path) -> dict:
    ...
    try:
        # pipeline/ is already on sys.path when running pipeline/status.py
        scripts_dir = str(repo_root / "pipeline")
