...
def _evidence_ref(root: Path) -> str:
    fields = _checkpoint_fields(
        root,
        Checkpoint="checkpoint-evidence-source",
        Owner="claude",
        Objective="preserve observed checkpoint evidence",
        **{
            "Accepted scope": "the throwaway checkpoint fixture repository",
            "Verification status": "the evidence observation is recorded",
            "Next action": "cite this checkpoint from the continuation record",
        },
    )
    relative = _publish(
        root, "claude", "codex", "findings",
        "2026-08-12T00-00-01Z", _body(fields),
    )
...
def test_checkpoint_round_trips_through_production_finalizer(tmp_path: Path) -> None:
    ...
    assert [item.event.path for item in committed] == [
        evidence.rsplit("@", 1)[0],
        relative,
    ]


def test_generic_findings_are_refused_without_publication(tmp_path: Path) -> None:
    root = _repo(tmp_path)
    _refuse(
        root, "claude", "codex", "findings",
        ...
        match="fully typed real checkpoint",
    )


def test_owner_must_match_envelope_sender(tmp_path: Path) -> None:
    root = _repo(tmp_path)
    fields = _checkpoint_fields(root, Owner="claude")
    _refuse(
        root, "codex", "all", "findings",
...
def test_unresolvable_evidence_ref_is_refused_at_publication(
    ...
    ghost = (
        ...
        "2026-08-01T00-00-00Z-claude-to-codex-findings.md@" + "b" * 40
    )
