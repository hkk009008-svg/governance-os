...
def test_automatic_task_routing_model_is_direct_deduplicated_and_effect_free() -> None:
    ...
    required = (
        ...
        "exactly one bounded snapshot",
        "read_thread(turnLimit=1, includeOutputs=false)",
        "bounded cadence",
        "immutable Git and mailbox artifacts",
        "do not repeat thread snapshots",
        "snapshot and immutable artifact reconciliation",
        "at most one normal discovery",
        "never resends the trigger",
        "asks the user to relay the trigger",
...
def test_automatic_task_routing_adapters_are_thin_and_synced() -> None:
    required = (
        ...
        "per-target cursor",
        "normal timeout continues wait_threads",
        "same cursor",
        "exactly one bounded",
        "read_thread(turnLimit=1, includeOutputs=false)",
        "immutable Git/mailbox artifacts",
        "do not repeat thread snapshots",
